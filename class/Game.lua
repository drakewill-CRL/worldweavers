...
function _M:newGame()
	...
	local birth = Birther.new(nil, self.player, {"base", "difficulty",  }, function()
...
function _M:display(nb_keyframe)
	...
	-- Tooltip is displayed over all else, but put in lower-right corner.
...
function _M:setupMouse(reset)
	...
	self.mouse:registerZone(self.hotkeys_display.display_x, self.hotkeys_display.display_y, self.w, self.h, function(button, mx, my, xrel, yrel, bx, by, event)
		self.hotkeys_display:onMouse(button, mx, my, event == "button", function(text) self.tooltip:displayAtMap(nil, nil, self.w, self.h, tostring(text)) end) --This only shows the tooltip for an ability on mouseover. Not the map tooltip.
	end)
